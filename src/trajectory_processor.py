...
class TrajectoryProcessor(map_processor.MapProcessor):
    ...
    def compute_possible_set(self, prior, delta=0):#计算δ-位置集合
        
        if delta == 0:
            state_nos = np.where(prior>0)[0]#这里用的是0-位置集合，即所有可能的位置数据【先验概率>0】
    ...
    def make_transmat_from_state_trajs(self, state_trajs):
        transition_mat = np.zeros((self.n_state, self.n_state))#按照大小初始化
        for state_traj in state_trajs:
            pre_state = state_traj[0] #前一时刻位置状态
    ...
    def _normalize(self, transition_mat):#归一化【转化成概率形式即马尔可夫概率转移矩阵】
